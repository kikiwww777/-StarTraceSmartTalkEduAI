...
async function buildPayload(
  ...
  const env = (executionData.environment || {}) as Record<string, unknown>
  const triggerObj = (executionData.trigger || {}) as Record<string, unknown>
  const triggerData = (triggerObj.data || {}) as Record<string, unknown>
  ...
  const payload: NotificationPayload = {
    ...
    timestamp,
    data: {
      workspaceId: (env.workspaceId as string | undefined) || undefined,
  ...
  // Include token usage in a stable location for downstream systems (e.g. EDUAI).
  ...
  // Mirror key scalar metrics (duration/cost) in a simple object.
  payload.data.metrics = {
    durationMs: log.totalDurationMs,
    costUsd: typeof cost.total === 'number' ? cost.total : undefined,
  }

  // EDUAI-specific metadata (studentId/courseId/classId/attemptId) can be injected via triggerData.metadata.
  if (eduMetadata && typeof eduMetadata === 'object') {
    payload.data.metadata = eduMetadata
  }
