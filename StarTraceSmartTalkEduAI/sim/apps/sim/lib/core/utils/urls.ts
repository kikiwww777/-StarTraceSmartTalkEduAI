...
export function getBaseUrl(): string {
  ...
  if (!baseUrl) {
    // Developer-friendly fallback: allow local dev to boot without explicit env.
    // Production should still set NEXT_PUBLIC_APP_URL explicitly.
    if (!isProd) {
      return 'http://localhost:3000'
    }
    throw new Error('NEXT_PUBLIC_APP_URL must be configured for webhooks and callbacks to work correctly')
  }
