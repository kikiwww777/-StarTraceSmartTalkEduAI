...
function parseSpaceOrCommaSeparatedList(value: string | undefined): string[] {
  if (!value) return []
  return value
    .split(/[,\s]+/g)
    .map((s) => s.trim())
...
function getEmbedFrameAncestors(): string[] {
  ...
  // Always include common development origins for easier local testing
  // This ensures iframe embedding works even if environment variables aren't set
  // Note: CSP frame-ancestors doesn't support wildcard ports (*), so we include common ports
  // Add a wide range of common development ports to support localhost:* pattern
  const commonPorts = [
    3000, 3001, 3002, 3003, 3004, 3005,  // Common Next.js/React dev servers
    5173, 5174, 5175, 5176, 5177, 5178,  // Vite default and common alternates
    8080, 8081, 8082, 8083,               // Common backend/API ports
    4000, 4001, 4002,                    // Alternative dev server ports
    5000, 5001, 5002,                    // Alternative dev server ports
    8000, 8001, 8002,                    // Alternative dev server ports
    9000, 9001, 9002,                    // Alternative dev server ports
  ]
  
  const devDefaults: string[] = []
  
  // Generate localhost and 127.0.0.1 variants for all common ports
  for (const port of commonPorts) {
    devDefaults.push(`http://localhost:${port}`)
    devDefaults.push(`http://127.0.0.1:${port}`)
  }
  
  // Remove duplicates and sort for consistency
  const uniqueDevDefaults = Array.from(new Set(devDefaults))

  // CSP frame-ancestors expects origins (scheme + host[:port]) or special keywords.
  // Keep 'self' always, then add configured/dev origins.
  // Always include dev defaults to ensure local development works
  const allOrigins = configured.length > 0 
    ? [...configured, ...uniqueDevDefaults]
    : uniqueDevDefaults

  return Array.from(new Set(["'self'", ...allOrigins]))
}
...
export function generateRuntimeCSP(): string {
  ...
  // Build frame-ancestors directive with allowed origins
  const frameAncestors = getEmbedFrameAncestors().join(' ')
...
/**
 * CSP policy for iframe-embedded pages (e.g. /embed/*).
 *
 * - Keeps the base policy aligned with the main app
 * - Overrides `frame-ancestors` to explicitly allow EDUAI parent origins
 */
export function getEmbedCSPPolicy(): string {
