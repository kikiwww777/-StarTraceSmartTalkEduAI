...
// Course role enumeration
export const courseRoleEnum = pgEnum('course_role', ['teacher', 'student', 'admin'])
...
export const course = pgTable(
  ...
  (table) => ({
    instructorIdIdx: index('course_instructor_id_idx').on(table.instructorId),
    workspaceIdIdx: index('course_workspace_id_idx').on(table.workspaceId),
    workspaceInstructorIdx: index('course_workspace_instructor_idx').on(
      table.workspaceId,
      table.instructorId
    ),
  })
...
export const courseMember = pgTable(
  ...
  (table) => ({
    courseIdIdx: index('course_member_course_id_idx').on(table.courseId),
    userIdIdx: index('course_member_user_id_idx').on(table.userId),
    courseUserIdx: index('course_member_course_user_idx').on(table.courseId, table.userId),
    roleIdx: index('course_member_role_idx').on(table.role),
    // Ensure a user can only be a member once per course
    uniqueCourseUserConstraint: uniqueIndex('course_member_course_user_unique').on(
      table.courseId,
      table.userId
    ),
  })
