-- 课程相关表结构（修复版）
-- 执行步骤：
-- 1. 先执行此SQL创建表（不包含users表的外键约束）
-- 2. 检查users表的id字段类型：SHOW CREATE TABLE users;
-- 3. 根据users表的id类型，执行对应的ALTER TABLE语句添加外键约束

-- ============================================
-- 步骤1：创建表（不包含users表的外键约束）
-- ============================================
...
-- ============================================
-- 步骤3：根据users表的id类型，选择对应的SQL执行
-- ============================================

-- 情况A：如果users表的id是 BIGINT（SIGNED，默认）
-- 执行以下SQL添加外键约束：
/*
...
-- 情况B：如果users表的id是 BIGINT UNSIGNED
-- 先修改字段类型，再添加外键约束：
/*
ALTER TABLE `course_groups` 
MODIFY `teacher_id` BIGINT UNSIGNED NOT NULL COMMENT '教师ID，外键关联users表';
...
ALTER TABLE `student_course_relations` 
MODIFY `student_id` BIGINT UNSIGNED NOT NULL COMMENT '学生ID，外键关联users表';
...
-- ============================================
-- 快速修复方案（如果users表id是BIGINT，直接执行）：
-- ============================================
-- 取消下面的注释并执行：

-- ALTER TABLE `course_groups` ADD CONSTRAINT `fk_course_groups_teacher` FOREIGN KEY (`teacher_id`) REFERENCES `users` (`id`) ON DELETE CASCADE;
-- ALTER TABLE `student_course_relations` ADD CONSTRAINT `fk_student_course_student` FOREIGN KEY (`student_id`) REFERENCES `users` (`id`) ON DELETE CASCADE;
